...
3. Upload the code:
   - Option A: Zip this repository (or just `src/`, `index.js`, and `package.json`) and upload via **Code → Upload from → .zip file**.
...
5. In **Configuration → General configuration**, set the handler to `index.handler`. A root `index.js` file proxies to the Smart Home handler in `src/index.js` so you do not need to change the folder structure.
...
Example Lambda test event for discovery:

```json
{
  "directive": {
    "header": { "namespace": "Alexa.Discovery", "name": "Discover", "messageId": "pws-test" },
    "payload": {}
  }
}
```

If you see `Missing Alexa Smart Home directive`, double-check that your test event follows the structure above and that your Lambda trigger is **Alexa Smart Home**. If you see `Directive header must include namespace and name`, ensure your event includes both fields under `directive.header` (e.g., `Alexa.Discovery`/`Discover` or `Alexa`/`ReportState`).
...
```bash
node -e "const h=require('./index'); h.handler({directive:{header:{namespace:'Alexa.Discovery',name:'Discover'},payload:{} } }).then(console.log)"
```
