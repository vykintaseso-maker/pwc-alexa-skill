...
Use these payloads in the Lambda console **Test** tab to validate each flow without waiting on Alexa:

- **Discovery** (expects the Smart Home trigger to be configured):
...
- **ReportState** (replace `endpointId` with one of the five devices and `token`/`correlationToken` with real values from Alexa):
...
```json
{
  "directive": {
    "header": {
      "namespace": "Alexa",
      "name": "ReportState",
      "messageId": "test-report-1",
      "correlationToken": "Atza|IQEBLjAsAhRm...",
      "payloadVersion": "3"
    },
    "endpoint": {
...
- **EventBridge schedule invocation** (verifies the 3-minute proactive push logic without a directive):
...
```json
{
  ...
  "region": "us-east-1",
  "resources": [
    "arn:aws:events:us-east-1:123456789012:rule/pws-every-3-minutes"
  ],
  "detail": {}
}
```

For ReportState tests, the `token` and `correlationToken` fields must come from a real Alexa invocation (e.g., captured in CloudWatch Logs) or the Alexa developer console's Test tab; fake tokens will cause authorization errors, but the handler logic and mapping still execute.
